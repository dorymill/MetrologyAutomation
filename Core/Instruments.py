...
class HP53132A(Init):
    ...
    def input_coupling(self,channel=1, type='AC'):
        self.std.write(f'INP{channel}:COUP {type}')
    ...
    def frequency_mode(self, channel=1, gate=1):
        ...
        self.std.write(f'SENS:FREQ:ARM:STOP:TIM {gate}')
        self.std.write('INIT:CONT ON')

    def rise_mode(self):
        self.std.write(f'SENS:FUNC:ON ":RISE:TIME 1"')
        self.std.write('INIT:CONT ON')

    def fall_mode(self):
        self.std.write('SENS:FUNC:ON ":FALL:TIME 1"')
        self.std.write('INIT:CONT ON')

    def read(self):
        return float(self.std.query('FETC?'))
